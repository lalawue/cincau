...
router:get("/images/:filename/", function(config, req, response, params)
    MasterPage.staticContent(config, req, response, "datas/images/" .. params.filename, "image/png")
end)

router:get("/css/:filename/", function(config, req, response, params)
    MasterPage.staticContent(config, req, response, "datas/css/" .. params.filename)
end)

router:get("/js/:filename/", function(config, req, response, params)
    MasterPage.staticContent(config, req, response, "datas/js/" .. params.filename, "application/javascript")
end)
...
function router:pageNotFound(config, req, response, params)
    if req.path == "/favicon.ico" then
        MasterPage.staticContent(config, req, response, "datas/images/favicon.png", "image/png")
    else
